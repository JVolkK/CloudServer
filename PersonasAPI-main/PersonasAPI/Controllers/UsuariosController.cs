...
namespace PersonasAPI.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        public async Task<ActionResult> Register(UsuariosDto usuariosDto)
        {
            ...
            {
                _response.IsSuccess = false;
                _response.DisplayMessage = "ERROR al crear el ususario";
                return BadRequest(_response);
            }
        ...
        [HttpPost("Login")] public async Task<ActionResult> Login(UsuariosDto user)
        {
            var resultado = await _usuarioRepositorio.Login(user.UserName, user.Password);
            if (resultado == "nouser")
            {
                _response.IsSuccess = false;
                _response.DisplayMessage = "Usuario no existente";
                return BadRequest(_response);
            }
